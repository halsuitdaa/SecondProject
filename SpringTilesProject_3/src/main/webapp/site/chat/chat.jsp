...
{
	 name=$('#name').val();
	 if(name.trim()==="") //이름이입력되야 소켓연결
	 {
		 $('#name').focus();
		 return;
	 }
	 websocket=new WebSocket("ws://localhost:8080/web/site/chat/chat-ws"); //endpoint랑 일치가 되야됨(/site/chat/chat-ws)
	 //소켓생성
...
function onMessage(event)
{
	...
	if(data.substring(0,4)=='msg:') //0~3번은 기능  문자는 4번부터
	{
...
function appendMessage(msg)
{
	...
	let m=$('#recvMsg').height()-ch //문자출력하는위치에
	$('#chatArea').scrollTop(m) //스클롤바를 갖다놔라
}
function send()
{
	let msg=$('#sendMsg').val()
